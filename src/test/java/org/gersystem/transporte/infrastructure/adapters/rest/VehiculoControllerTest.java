...
//        vehiculoDTO.setCapacidad(new BigDecimal("1000"));
        ...
        // Act & Assert
        mockMvc.perform(get("/api/v1/vehiculos/{id}", vehiculoId).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(jsonPath("$.id").value(vehiculoId)).andExpect(jsonPath("$.placa").value("ABC-123"));
    }
    ...
    void debeDevolver401CuandoNoAutenticado() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get("/api/v1/vehiculos/{id}", vehiculoId).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isUnauthorized());
    }
} 